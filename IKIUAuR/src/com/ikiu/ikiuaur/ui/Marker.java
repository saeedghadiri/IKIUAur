...
import com.ikiu.ikiuaur.camera.CameraModel;
import com.ikiu.ikiuaur.common.Vector;
import com.ikiu.ikiuaur.data.ARData;
import com.ikiu.ikiuaur.data.PhysicalLocation;
import com.ikiu.ikiuaur.uiobjects.PaintableBoxedText;
import com.ikiu.ikiuaur.uiobjects.PaintableCircle;
import com.ikiu.ikiuaur.uiobjects.PaintableGps;
import com.ikiu.ikiuaur.uiobjects.PaintableObject;
import com.ikiu.ikiuaur.uiobjects.PaintablePoint;
import com.ikiu.ikiuaur.uiobjects.PaintablePosition;

/**
 * This class gets the location and other sensors data and calculates the
 * markers on screen location and draws its text This class will represent a
 * physical location and will calculate it's visibility and draw it's text and
 * visual representation accordingly. This should be extended if you want to
 * change the way a Marker is viewed.
 * 
 * 
 */
...
public class Marker implements Comparable<Marker> {
	...
	public Marker(String name, double latitude, double longitude,
			double altitude, int color, Typeface face) {
		set(name, latitude, longitude, altitude, color, face);
	}
	...
	private void updateView() {
		...
		// TODO: Revisit for a better approach, I assume it's a "square" axis
		// aligned square.
		...
		float lrX = x + max / 2;
		float lrY = y + max / 2;
		if (lrX >= -1 && ulX <= cam.getWidth() && lrY >= -1
				&& ulY <= cam.getHeight())
			isInView = true;
	}
	...
	public boolean handleClick(float x, float y) {
		...
		Log.i("handleClick", "point (x=" + x + " y=" + y + ") isPointOnMarker="
				+ result);
		return result;
	}
	...
	protected void drawIcon(Canvas canvas) {
		...
		if (ARActivity.useMarkerAutoRotate) {
	...
	private void drawText(Canvas canvas) {
		...
		textStr.setLength(0);
		if (distance < 1000.0) {
			textStr.append(name).append(" (").append("m ")
					.append(DECIMAL_FORMAT.format(distance)).append(")");
		} else {
			double d = distance / 1000.0;
			textStr.append(name).append(" (").append("km ")
					.append(DECIMAL_FORMAT.format(d)).append(")");
		}
		float maxHeight = Math.round(canvas.getHeight() / 10f) + 1;

		if (textBox == null)
			textBox = new PaintableBoxedText(textStr.toString(),
					Math.round(maxHeight / 2f) + 2, 300, face);
		else
			textBox.set(textStr.toString(), Math.round(maxHeight / 2f) + 2, 300);
	...
	private void drawPosition(Canvas canvas) {
		...
		if (positionContainer == null)
			positionContainer = new PaintablePosition(positionPoint, x, y,
					currentAngle, 1);
		else
	...
	private static final class Box {
		...
		private void set(PaintableObject paintable) {
			...
			if ((paintable instanceof PaintableGps)
					|| (paintable instanceof PaintableCircle)) {
				// drawing circles is slightly different then anything else,
		 ...
		 */
		private static final byte side(float Ax, float Ay, float Bx, float By,
				float x, float y) {
		 ...
		 */
		private static final boolean between(float aX, float aY, float bX,
				float bY, float cX, float cY, float dX, float dY, float x,
				float y) {
		...
		private boolean isPointInBox(float xPoint, float yPoint) {
			...
			boolean betweenTB = between(ulX, ulY, urX, urY, llX, llY, lrX, lrY,
					xPoint, yPoint);
			// Is the point between the left and right lines
			boolean betweenLR = between(ulX, ulY, llX, llY, urX, urY, lrX, lrY,
					xPoint, yPoint);
			if (betweenTB && betweenLR)
				return true;
			return false;
		}
